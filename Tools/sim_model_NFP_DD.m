function [dS_ddelta,dS_dTheta] =sim_model_NFP_DD(expdelta,Theta2)
...
parfor i=1:length(locNodeDist)
    ni=locNodeDist(i);
    sList=find(cutNodes(:,1)<=ni & cutNodes(:,2)>=ni);
    market = nodeMarket(i);
     
    %dist_ni=distanceMatrix(:,ni);
    dist_ni = D.Value(:,ni);
    wn=WNodes(ni,:);
    weights=wn(:,5:end);
    ...
    for s=sList' % Parallel work across markets
        ...
        if TypeD==7 || TypeD==8 || TypeD==9
            ...
            dni1p = 0; dni2p = 0; dni2l = 0; dni3p = 0; dni4p = 0;           
        end     
...
for S=1:length(locNodeDist)
    slist=Model.mapNodeList(S,:);
    for y=1:length(Model.Years)
        
        pickS=(slist(y) ==counterMarket);
        
        dS_dTheta(pickS,:) = dS_dTheta(pickS,:) + dS_dTh{S,1}{y} ;
        dS_ddelta(pickS,pickS) = dS_ddelta(pickS,pickS)+ dS_dd{S}{y};
    end
end
